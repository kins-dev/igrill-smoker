...
class SSR(object):
    ...
    def StartThread(self):
        ...
        pi = pigpio.pi()
        item = constant.SSR_CONTROL_BOARD_ITEMS["SSR"][self.m_boardVal]
        offVal = 1000000
        onVal = offVal // 2
...
def main():
    ...
    daemon = Daemon(host=constant.SSRC_DAEMON_PYRO_HOST,
                    port=constant.SSRC_DAEMON_PYRO_PORT)
    ssrcObj = SSR(daemon)
    uri = daemon.register(
        ssrcObj, objectId=constant.SSRC_DAEMON_PYRO_OBJECT_ID)
    logging.debug(uri)
    daemon.requestLoop()
