...
RUN_LIMITS="${IGRILL_RUN_DIR}/${iGrill__RuntimeFiles__LimitsFile}"
RUN_DATA="${IGRILL_RUN_DIR}/${iGrill__RuntimeFiles__TempDataFile}"
BAD_DATA=-2000
TIME=-1
TIMESTAMP=0
...
if [ "Mini" == "${iGrill__iGrill__Type}" ]; then
    ...
    if ! [ "1" == "${iGrill__Probes__SmokeProbe}" ]; then
...
if [ "0" -gt "${iGrill__Probes__FoodProbe}" ] || [ "4" -lt "${iGrill__Probes__FoodProbe}" ]; then
    echo "Error: Food probe must be set between 0 and 4 in iGrill_config.ini"
    exit 1
fi
if [ "1" -gt "${iGrill__Probes__SmokeProbe}" ] || [ "4" -lt "${iGrill__Probes__SmokeProbe}" ]; then
    echo "Error: Smoke probe must be set between 0 and 4 in iGrill_config.ini"
    exit 1
fi
if [ "${iGrill__Probes__FoodProbe}" == "${iGrill__Probes__SmokeProbe}" ]; then
